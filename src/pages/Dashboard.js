...
const Dashboard = () => {
  ...
  useEffect(() => {
    if (user?.role === "worker") {
      axios.get("/bookings/worker/my-bookings", {
        headers: { Authorization: `Bearer ${token}` },
      })
  ...
  const fetchAdminData = async (type) => {
    ...
    try {
      if (type === "users") {
        const res = await axios.get("/bookings/admin/users", {
          headers: { Authorization: `Bearer ${token}` },
        });
        setUsers(res.data);
      }
      ...
      if (type === "bookings") {
        ...
        const userRes = await axios.get("/bookings/admin/users", {
          headers: { Authorization: `Bearer ${token}` },
        });
      ...
      if (type === "services") {
        const res = await axios.get("/bookings/admin/services", {
          headers: { Authorization: `Bearer ${token}` },
        });
  ...
  const handleDeleteUser = async (id) => {
    ...
    try {
      await axios.delete(`/bookings/admin/users/${id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handleDeleteService = async (id) => {
    ...
    try {
      await axios.delete(`/bookings/admin/services/${id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handleEditService = (service) => {
    ...
    axios.put(`/bookings/admin/services/${service.id}`, {
      name: newName,
  ...
  const handleAddService = async () => {
    ...
    try {
      await axios.post("/bookings/admin/services", newService, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const assignWorker = async (bookingId, workerId) => {
    try {
      await axios.put(`/bookings/admin/assign-worker/${bookingId}`, {
        worker_id: workerId
      }, {
        headers: { Authorization: `Bearer ${token}` }
      });
